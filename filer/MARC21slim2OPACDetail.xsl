...
<xsl:stylesheet version="1.0"
 ...
 <xsl:when test="$leader6='m'">Datorfil </xsl:when>
 ...
 <xsl:with-param name="label">Upplaga: </xsl:with-param>
 </xsl:call-template>
 </xsl:if>

 <xsl:if test="marc:datafield[@tag=250]">
 <span class="results_summary edition"><span class="label">Upplaga: </span>
 ...
 <span class="label">Mediatyp:</span>
 ...
 <xsl:when test="@ind2=6"> [Rubrik]</xsl:when>
 <xsl:when test="@ind2=7"> [Kolumntitel]</xsl:when>
 ...
 <span class="results_summary translated_title"><span class="label">Titel översatt:</span>
 ...
 <xsl:with-param name="label">Uniforma titlar: </xsl:with-param>
 </xsl:call-template>
 </span>
 </xsl:if>

 <xsl:if test="marc:datafield[@tag=130]|marc:datafield[@tag=240]|marc:datafield[@tag=730][@ind2!=2]">
 <span class="results_summary uniform_titles"><span class="label">Uniforma titlar: </span>
 ...
 <xsl:for-each select="marc:datafield[substring(@tag, 1, 1) = '6'][not(@tag=653) and not(@tag=655) and not(@tag=681)]">
 ...
 <xsl:otherwise> | </xsl:otherwise>
        ...
        <!-- New 653 loop -->
 <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and @tag=653]">
                <xsl:choose>
                  <xsl:when test="position()=last()"></xsl:when>
                  <xsl:otherwise> - </xsl:otherwise>
                </xsl:choose>
        <xsl:for-each select="marc:datafield[substring(@tag, 1, 1) = '6'][@tag=653]">
           <xsl:for-each select="marc:subfield">
			   <!-- <xsl:for-each select="marc:subfield[@code='a']"> -->
              <span property="keywords"><a>
                <xsl:attribute name="class">subject</xsl:attribute>
                   <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=su<xsl:value-of select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/>
                     <xsl:value-of select="text()"/>
                     <xsl:value-of select="$TracingQuotesRight"/></xsl:attribute>
                     <xsl:value-of select="text()"/>
                </a>
         ...
         <!-- New 681 loop -->
     <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and @tag=681]">
                 <xsl:choose>
                   <xsl:when test="position()=last()"></xsl:when>
                   <xsl:otherwise> / </xsl:otherwise>
                 </xsl:choose>
         <xsl:for-each select="marc:datafield[substring(@tag, 1, 1) = '6'][@tag=681]">
            <xsl:for-each select="marc:subfield">
          <!-- <xsl:for-each select="marc:subfield[@code='a']"> -->
               <span property="keywords"><a>
                 <xsl:attribute name="class">subject</xsl:attribute>
          ...
          <!-- New 886 loop -->
      <xsl:if test="marc:datafield[@tag=886]">
                  <xsl:choose>
                    <xsl:when test="position()=last()"></xsl:when>
                    <xsl:otherwise> + </xsl:otherwise>
                  </xsl:choose>
        <xsl:for-each select="marc:datafield[@tag=886]">
            <xsl:for-each select="marc:subfield[@code='a']=861">
             <xsl:for-each select="marc:subfield">
                <span property="keywords"><a>
                  <xsl:attribute name="class">subject</xsl:attribute>
                     <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=su<xsl:value-of select="$SubjectModifier"/>:<xsl:value-of select="$TracingQuotesLeft"/>
...
<!-- Image processing code added here, takes precedence over text links including y3z text -->
 ...
 <span class="results_summary online_resources"><span class="label">Online-resurser: </span>
 ...
 <span class="label">Dissertationskommentar: </span>
 ...
 <xsl:when test="@ind1=3"><xsl:text>Sammandrag: </xsl:text></xsl:when>
